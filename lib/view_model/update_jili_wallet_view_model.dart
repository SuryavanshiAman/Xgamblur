// import 'package:flutter/foundation.dart';
// import 'package:flutter/material.dart';
// import 'package:bettsix/repo/update_jili_wallet_repo.dart';
// import 'package:bettsix/res/color-const.dart';
// import 'package:bettsix/utils/routes/routes_name.dart';
// import 'package:bettsix/utils/utils.dart';
// import 'package:bettsix/view_model/user_view_model.dart';
//
// class UpdateJiliWalletViewModel with ChangeNotifier {
//   final _updateJiliWalletRepo = UpdateJiliWalletRepo();
//
//   bool _loading = false;
//
//   bool get loading => _loading;
//
//
//   setLoading(bool value) {
//     _loading = value;
//     notifyListeners();
//   }
//
//   Future<void> updateJiliWallletApi( context) async {
//     UserViewModel userViewModal = UserViewModel();
//     String? userId = await userViewModal.getUser();
//     setLoading(true);
//     Map data={
//       "user_id":userId,
//     };
//     _updateJiliWalletRepo.updateJiliWallletApi(data).then((value) {
//       if (value['status'] == 200) {
//         setLoading(false);
//         Utils.setSnackBar(value['message'], AppColor.green, context);
//       }
//       else {
//         setLoading(false);
//         Utils.setSnackBar(value['message'], AppColor.red, context);
//       }
//     }).onError((error, stackTrace) {
//       setLoading(false);
//       if (kDebugMode) {
//         print('updateJilliToUserWalletApi: $error');
//       }
//     });
//   }
// }